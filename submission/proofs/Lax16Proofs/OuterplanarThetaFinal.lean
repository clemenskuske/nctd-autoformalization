...
lemma exists_firstReturnEar_of_adj_outside
  ...
  obtain ⟨q, hq, hxq⟩ :=
    Lax16ThetaDirect.exists_path_avoiding_vertex G htwo hyx hleftx
  have hleftS : T.left ∈ T.vertexSet := T.left_mem_vertexSet
  obtain ⟨z, hzS, r, hr, hrfirst, hrsubq⟩ :=
    Lax16ThetaDirect.exists_initial_path_to_set
  ...
  · by_cases hzright : z = T.right
    ...
    · have hxint :
      ...
      have hdk : d ≠ k := by
        ...
        apply hdj
        exact h.trans hkJ
      exact Lax16ThetaK4.hasTopologicalModel_k4_of_cross_arm_ear
    ...
    let E : ThetaEar T :=
      { start := (T.route i).snd
        ...
        interior_avoids := by
          ...
          · exact hw.2.1 hwx
          · exact hw.2.2 hwz }
    exact Lax16ThetaSameArmUse.false_of_rank_maximal_same_arm_ear
...
lemma rank_maximal_escape
  ...
  exact Lax16ThetaLeftReturnUse.false_of_rank_maximal_left_return_ear
...
lemma min_degree_three_false_of_rank_maximal_escape
  ...
  obtain ⟨T₀, -⟩ :=
    Lax16ThetaDirect.exists_theta_of_degree_three G htwo
