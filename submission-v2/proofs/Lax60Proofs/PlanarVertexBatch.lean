...
namespace Lax60Proofs.PlanarVertexBatch
...
/--
---
conclusion: Lax60.PlanarVertexBatch.exists_planar_vertex_batch_of_degree_ge_four
assumptions:
  - Lax60.OuterplanarLayer.exists_outerplanar_layer_assignment
---
...
theorem exists_planar_vertex_batch_of_degree_ge_four {V : Type u}
  ...
  obtain ⟨W, hWcard, hWneighbors, hWseparates⟩ :=
    Lax60Proofs.CenterSet.exists_center_set G hplanar v hdegree
  ...
  obtain ⟨hYunique, hYgadget⟩ :=
    Lax60Proofs.OutsideThreeSetGadget.outside_three_set_gadget
  ...
  obtain ⟨D, hD⟩ :=
    Lax60Proofs.DominatorCompletion.exists_dominator_completion
      G hplanar v hdegree
  ...
  have houterX :
    ...
    have hXset : (↑X : Set V) = G.neighborSet v := by
      ...
      simp [X]
    rw [hXset]
    exact Lax60Proofs.PlanarToolbox.neighbor_layer_outerplanar hplanar v
...
end Lax60Proofs.PlanarVertexBatch
