...
namespace Lax60Proofs.OuterplanarBound
...
private def liftInducedModel {W : Type u} {V : Type v}
  ...
  route h :=
    Lax60Proofs.PlanarToolbox.liftInducedWalk (M.route h)
  route_isPath h := by
  ...
  branch_avoids_interiors h w := by
    change (M.branch w).1 ∉
      walkInterior
        (Lax60Proofs.PlanarToolbox.liftInducedWalk (M.route h))
    rw [Lax60Proofs.PlanarToolbox.walkInterior_liftInducedWalk
      (M.route h)]
    rintro ⟨x, hx, hxeq⟩
  ...
  route_interiors_disjoint hab hcd hne := by
    have hi₁ :
        walkInterior
            (Lax60Proofs.PlanarToolbox.liftInducedWalk (M.route hab)) =
          Subtype.val '' walkInterior (M.route hab) :=
      Lax60Proofs.PlanarToolbox.walkInterior_liftInducedWalk _
    have hi₂ :
        walkInterior
            (Lax60Proofs.PlanarToolbox.liftInducedWalk (M.route hcd)) =
...
private theorem connected_outerplanar_exact_two
  ...
  | h n ih =>
    ...
    · have hn_three : 3 ≤ n := by omega
      ...
      · exact
          Lax60.OuterplanarBlockAssignment.exists_two_label_assignment
            G houter htwo
      · have hcut :
...
/--
---
conclusion: Lax60.OuterplanarBound.positiveNCTD_le_two
assumptions:
  - Lax60.OuterplanarBlockAssignment.exists_two_label_assignment
---
...
theorem positiveNCTD_le_two {V : Type u} [Fintype V]
  ...
  have hglobal : HasPositiveNCTDAtMost G 1 2 :=
    Lax60Proofs.ComponentReduction.combine_components G 2 hcomponents
  unfold positiveNCTD
  exact Nat.sInf_le hglobal

end Lax60Proofs.OuterplanarBound
