...
namespace Lax60Proofs.DominatorCompletion
...
lemma exists_dominator_completion {V : Type u} [Fintype V]
  ...
  have singleton_completion (w : V)
    ...
    · obtain ⟨S, hS⟩ :=
        Lax60Proofs.UniqueDominator.exists_unique_dominator_set
  ...
  have hbounds :=
    Lax60Proofs.DominatorCount.dominator_count_bounds G hplanar v
  ...
  · have hcard_le : (dominatorSet G v).ncard ≤ 2 :=
    ...
    · obtain ⟨w₁, w₂, hne, hdom⟩ := Set.ncard_eq_two.mp htwo
      exact
        Lax60Proofs.TwoDominators.exists_two_dominator_assignment
...
end Lax60Proofs.DominatorCompletion
